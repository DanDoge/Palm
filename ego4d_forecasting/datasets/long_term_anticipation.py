...
class Ego4dLongTermAnticipation(torch.utils.data.Dataset):
    def __init__(self, cfg, mode):
        ...
        if mode == 'test' and cfg.TEST.EVAL_VAL:
            mode_ = 'val'
        if mode == 'train' and cfg.TRAIN.SUBSAMPLE:
            print("loading subsampled training set")
            mode_ = 'train_small'
        if mode == 'val' and cfg.TRAIN.SUBSAMPLE:
            print("loading subsampled validation set")
            mode_ = 'val_small'
        data_path = os.path.join(self.cfg.DATA.PATH_TO_DATA_DIR, f'fho_lta_{mode_}.json')
        ...
        self._dataset_iter = itertools.chain.from_iterable(
            itertools.repeat(iter(self.dataset), 987654321) # not sure why though
        )
    ...
    def _make_transform(self, mode: str, cfg):
        ...
        return Compose(
            [
                Lambda(
                    lambda x: (
                        x["video"] if "video" in x else x["feature"],
