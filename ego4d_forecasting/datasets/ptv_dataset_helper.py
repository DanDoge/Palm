...
class LabeledFeatureDataset(torch.utils.data.IterableDataset):
    ...
    def load_feature(self, clip_uid, action_idx, i_try):
        filename_slowfast = "/PATH_TO_SLOWFAST_FEATURE/{}_{}".format(clip_uid, action_idx)
        filename_egovlp = "PATH_TO_EGOVLP_FEATURE/{}_{}.pt".format(clip_uid, action_idx)
        try:
            #with open(filename_slowfast, "rb") as f:
            #    slowfast_feature = pickle.load(f)
            egovlp_feature = torch.load(filename_egovlp)
            #egovlp_feature = egovlp_feature.sum(0) / (egovlp_feature[..., 0] != 0).sum().numpy()
            #loaded_feature = np.concatenate((slowfast_feature, egovlp_feature), axis=None)

            non_zero = (egovlp_feature[..., 0] != 0).sum().item()
            padded = torch.zeros(256 * 12)
            padded[:non_zero * 256] = egovlp_feature[:non_zero].reshape(-1)
            return padded
            loaded_feature = np.concatenate((slowfast_feature, padded), axis=None)
    ...
    def __next__(self) -> dict:
        ...
        for input_clip in info_dict["input_clips"]:
            ...
            for i_try in range(self._MAX_CONSECUTIVE_FAILURES):
                try:       
                    if input_clip["action_idx"] < self._MAX_CONSECUTIVE_FAILURES:
                        action_idx_load = input_clip["action_idx"] + i_try 
                    else:
                        action_idx_load = input_clip["action_idx"] - i_try 
                    loaded_feature = self.load_feature(input_clip["clip_uid"], action_idx_load, i_try)
                    if loaded_feature is None:
                        raise Exception()
                    decoded_features.append(loaded_feature)
                    find = True 
                    break 
                except Exception as e:
                    logger.debug(
            ...
            if not find:
                ...
                loaded_feature = self.load_feature("5085ff96-926e-4462-adb9-57f91215e881", 6, 0)
                decoded_features.append(loaded_feature)

        sample_dict = {
            "feature": decoded_features,
            "video_index": video_index,
            **info_dict,
        }
        if self._transform is not None:
