...
class LongTermAnticipationTask(VideoTask):
    ...
    def training_step(self, batch, batch_idx):
        ...
        preds = self.forward(input, tgts=observed_labels)
        ...
        for head_idx, pred_head in enumerate(preds):
            for seq_idx in range(pred_head.shape[1]):
                ...
                loss += self.loss_fun(
                    pred_head[:, seq_idx], observed_labels[:, seq_idx, head_idx]
                )
                top1_err, top5_err = metrics.distributed_topk_errors(
                    pred_head[:, seq_idx], observed_labels[:, seq_idx, head_idx], (1, 5)
                )
    ...
    def validation_step(self, batch, batch_idx):
        input, forecast_labels, observed_labels, _, label_clip_times = (
            batch
        ...
        for head_idx, pred in enumerate(preds):
            ...
            pred, observed_labels = pred.cpu(), observed_labels.cpu()

            label = observed_labels[:, :, head_idx : head_idx + 1]
        ...
        preds_action = preds[0] * 521 + preds[1]
        labels_action = observed_labels[:, :, 0:1] * 521 + observed_labels[:, :, 1:2]
        auedit = metrics.distributed_AUED(preds_action.permute(0, 2, 1).cpu(), labels_action.cpu())
        results = {
    ...
    def test_step(self, batch, batch_idx):
        input, forecast_labels, obesrved_labels, last_clip_ids, label_clip_times = (
            batch
        )  # forecast_labels: (B, Z, 1)
        k = self.cfg.FORECASTING.NUM_SEQUENCES_TO_PREDICT
